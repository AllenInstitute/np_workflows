...
def mtrain_regimen_1_enter(state):
    # current regimen is already set ion prev screen, but we set it again here anyway
    state["external"]["current_regimen"] = npxc.mtrain.regimen['name']
...
def mtrain_regimen_2_enter(state):     
    ...
    new_regimen_dict = [regimen for regimen in npxc.mtrain.get_all("regimens") if regimen['name'].lower() == state["external"]["new_regimen"].lower()][0]
...
def mtrain_regimen_2_input(state): 
    ...
    if cancel_regimen_select:
        ...
        new_stage_dict = [stage for stage in new_regimen_dict['stages'] if stage['name'].lower() == selected_stage_new_regimen.lower()][0]
