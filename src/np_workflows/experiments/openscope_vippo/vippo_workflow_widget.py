...
class SelectedSession:
    def __init__(self, session: str | VippoSession, mouse: str | int | np_session.Mouse):
...
def vippo_workflow_widget(
    ...
    selection = SelectedSession(VippoSession.PRETEST, mouse)
    ...
    console = ipw.Output()
    with console:
        if last_session := np_session.Mouse(selection.mouse).state.get('last_vippo_session'):
