...
def per_session_main_stim_params(session: BarcodeSession) -> dict[str, Any]:
    ...
    def session_stim_repeats(session: BarcodeSession) -> dict[str, dict[str, int]]:
        repeats = functools.partial(build_session_stim_params, "stim_repeats")
        match session:
            case BarcodeSession.PRETEST:
                return repeats(1, 1, 1)
            case BarcodeSession.HAB_60:
                return repeats(15, 5, 1)
            case BarcodeSession.HAB_90:
                return repeats(20, 7, 2)
            case BarcodeSession.HAB_120 | BarcodeSession.EPHYS:
                return repeats(25, 8, 2)
            case _:
                raise ValueError(f"Stim repeats not implemented for {session}")

    def session_stim_lengths(session: BarcodeSession) -> dict[str, dict[str, int]]:
    ...
    def main_blank_screen(session: BarcodeSession) -> dict[str, float | int]:
        match session:
            case BarcodeSession.PRETEST:
...
def per_session_opto_params(
    session: BarcodeSession, mouse: str | int | np_session.Mouse
    ...
    def opto_levels(session: BarcodeSession) -> dict[str, list[float]]:
...
def per_session_mapping_params(session: BarcodeSession) -> dict[str, dict[str, int]]:
    ...
    match session:
        case BarcodeSession.PRETEST | BarcodeSession.EPHYS | BarcodeSession.HAB_120:
            ...
            return params
        case BarcodeSession.HAB_60 | BarcodeSession.HAB_90:
            return {}
        case _:
            raise ValueError(f"Mapping params not implemented for {session}")
